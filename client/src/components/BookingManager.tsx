        ...
        <div className="space-y-4">
          {userBookingsQuery.data.map((booking) => (
            <Card key={booking.id} className="card-elevated p-6 hover:shadow-lg transition-all">
              ...
              {booking.completionCode && booking.status === 'confirmed' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-green-600 font-medium">Completion Code</p>
                      <p className="text-2xl font-mono font-bold text-green-600 mt-1">{booking.completionCode}</p>
                      <p className="text-xs text-green-700 mt-1">Share this with admin when you arrive</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
